...
class woedit extends StatefulWidget {
  static const String routeName = '/woedit';

  @override
  State<woedit> createState() => _woeditState();
}

class _woeditState extends State<woedit> {
  ...
  void _uploadFile() async {
    ...
    var request = http.MultipartRequest(
        'POST',
        Uri.parse(
            'http://147.139.134.143/mrtjakarta/public/api/preventiveopen/attach'));
    request.fields.addAll({'id-attach': globals.woexecuteidtemp.toString()});
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonDecode(data)['success'].toString() == 'false') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(jsonDecode(data)['successMsg'].toString()),
        ));
  ...
  Future<void> _deletetoolspreventive(String a) async {
    ...
    Navigator.of(context)
        .push(MaterialPageRoute(builder: (BuildContext context) => woedit()));
  }
    ...
    if (response.statusCode == 200) {
      ...
      if (json.decode(databack)!['success'] == false) {
        ...
        Navigator.of(context).push(
            MaterialPageRoute(builder: (BuildContext context) => woedit()));
      }
    } else {
      print(response.reasonPhrase);
    }
  ...
  Future<void> _addequipmentid() async {
    ...
    Navigator.of(context)
        .push(MaterialPageRoute(builder: (context) => woedit()));
    // Navigator.of(context).pop();
    // Navigator.of(context).push(
    //     MaterialPageRoute(builder: (BuildContext context) => executewo()));
  }
    ...
    setState(() {
      for (var i = 0; i < _getwonumber.length; i++) {
        ...
        if (globals.workorder_notemp.toString() ==
          ...
          initializeworkorder = _getwonumber[i]['name'].toString();
                            ...
                            onPressed: () {
                              if (selecttools == '') {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: const Text('Please Select Tools'),
                                  ),
                                );
                              } else {
                                _addTools();
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      width: MediaQuery.of(context).size.width -
                          (MediaQuery.of(context).size.width / 10),
                      // color: _files == null??Colors.accents:Colors.white,
                      child: OutlinedButton(
                    ...
                    FlatButton.icon(
                      onPressed: () {
                        // print(_files!.first.path);
                        if (_files!.first.path == null) {
                          // print('not selected');
                          // ScaffoldMessenger.of(context)
                          //     .showSnackBar(
                          //   SnackBar(
