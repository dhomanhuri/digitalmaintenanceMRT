...
Future<void> createExcel(List<dynamic> _getfailure) async {
  ...
  for (int i = 0; i < _getfailure.length; i++) {
    sheet.getRangeByName('B' + (i + 6).toString()).setText((i + 1).toString());
    sheet
        .getRangeByName('C' + (i + 6).toString())
        .setText(_getfailure[i]['title']);
    sheet
        .getRangeByName('D' + (i + 6).toString())
        .setText(_getfailure[i]['report_date']);
    sheet
        .getRangeByName('E' + (i + 6).toString())
        .setText(_getfailure[i]['departmentname']);
    sheet
        .getRangeByName('F' + (i + 6).toString())
        .setText(_getfailure[i]['equipment_idname'].toString());
    sheet
        .getRangeByName('G' + (i + 6).toString())
        .setText(_getfailure[i]['status'].toString());
  }
