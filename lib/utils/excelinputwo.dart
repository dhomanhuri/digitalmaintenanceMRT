...
Future<void> createExcel(List<dynamic> data) async {
  ...
  sheet.getRangeByName('F5').setText('Date Time');
  sheet.getRangeByName('F5').cellStyle.fontSize = 12;
  ...
  for (int i = 0; i < data.length; i++) {
    ...
    sheet
        ...
        .setText(data[i]['technician'].toString());
    sheet.getRangeByName('G' + (i + 6).toString()).setText('Start : ' +
        ' ' +
        data[i]['start_time'].toString() +
        '\n' +
        'End : ' +
        data[i]['end_time'].toString());
    if (data[i]['status'] == 1) {
      if (data[i]['status_proses'] == 0) {
        sheet.getRangeByName('H' + (i + 6).toString()).setText('Open');
      } else if (data[i]['status_proses'] == 1) {
        sheet.getRangeByName('H' + (i + 6).toString()).setText('In Progress');
      } else if (data[i]['status_proses'] == 2) {
        sheet
            .getRangeByName('H' + (i + 6).toString())
            .setText('Need - Approval');
      } else if (data[i]['status_proses'] == 3) {
        sheet.getRangeByName('H' + (i + 6).toString()).setText('Close');
      } else if (data[i]['status_proses'] == 4) {
        sheet.getRangeByName('H' + (i + 6).toString()).setText('Need-Revision');
      }
    } else {
      sheet.getRangeByName('H' + (i + 6).toString()).setText('Not Active');
    }
  }
